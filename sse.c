...
int main()
{
	...
	for (j=1; j<M; j++)
	{
		...
		for (i=1; i<N; i++)
		{
			*vb=_mm_add_ps(_mm_mul_ps(kk,*va), _mm_mul_ps(k,_mm_add_ps(fa[i-1-M], fa[i-M], fa[i+1-M], fa[i-1], fa[i+1], fa[i-1+M], fa[i+M], fa[i+1+M])); //o metasxhmatismos ginetai kata grammh
			va++;
			vb++;		
		}
	}
